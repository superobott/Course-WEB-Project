  ...
  lines.forEach(line => {
    ...
    if (match) {
      match.forEach(dateStr => {
        ...
        results.push({ date: dateStr, summary: line.trim(), sortDate: dateObj });
      });
    }
  });
...
function App() {
  ...
  <div className="min-h-screen bg-[#111827] text-white px-4 py-8">
    <div className="flex flex-col items-center">
      <h1 className="text-5xl font-bold text-[#00CED1] mb-6 text-center">
        Timeline Search
      </h1>
      <SearchBar onSearch={setQuery} />
      {query && !loading && (
        <h2 className="text-xl mt-4 text-[#00CED1]">{`"${query}"`}</h2>
      )}
    </div>
    ...
    {loading ? (
      ...
      <div className="relative mt-12 flex flex-col items-center">
        {/* קו אנכי במרכז */}
        <div className="absolute w-1 bg-[#00CED1] top-0 bottom-0 left-1/2 transform -translate-x-1/2" />
